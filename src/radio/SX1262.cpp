...
{
    _txp = 14;  // Base default TX power
    _instance = this;
    memset(_packet, 0, sizeof(_packet));
}

// =============================================================================
// Pre-Initialization — SPI bus setup + chip detection
// =============================================================================
...
bool SX1262::preInit() {
    ...
    // Retry sync word read for up to 2 seconds
    long start = millis();
    uint8_t syncmsb, synclsb;
...
bool SX1262::begin(uint32_t frequency) {
    ...
    Serial.printf("[SX1262] Post-cal DevErrors: 0x%04X%s\n",
        postCalErr, (postCalErr & 0x40) ? " *** PLL FAIL ***" : " OK");
    clearDeviceErrors();
    ...
    // Enable all IRQ sources in the IRQ mask — required for GetIrqStatus polling
    // Route RX_DONE to DIO1 for optional interrupt-driven RX
    uint8_t irqBuf[8];
    irqBuf[0] = 0xFF;  // IrqMask MSB — enable all
    irqBuf[1] = 0xFF;  // IrqMask LSB
    irqBuf[2] = 0x00;  // DIO1 mask MSB
    irqBuf[3] = IRQ_RX_DONE_MASK_6X;  // DIO1: RX done
    irqBuf[4] = 0x00;  // DIO2 mask MSB
    irqBuf[5] = 0x00;  // DIO2 mask LSB
    ...
    // Clear any latched device errors from init
    clearDeviceErrors();
...
int SX1262::parsePacket(int size) {
    ...
    if ((buf[1] & IRQ_RX_DONE_MASK_6X) == 0) {
        return 0;
    }
    ...
    if (!crcOk) {
        ...
        for (int i = 0; i < 16 && i < packetLength; i++) Serial.printf("%02X ", _packet[i]);
        Serial.println();
        receive();
        return 0;
    }
...
uint32_t SX1262::getFrequency() {
    return _frequency;
}

void SX1262::setTxPower(int level) {
    // Workaround: Better TX antenna mismatch resistance (DS_SX1261-2_V1.2 ch15.2)
    writeRegister(REG_TX_CLAMP_CONFIG_6X, readRegister(REG_TX_CLAMP_CONFIG_6X) | (0x0F << 1));

    uint8_t pa_buf[4];
    pa_buf[0] = 0x04;  // PADutyCycle for 22dBm
    pa_buf[1] = 0x07;  // HPMax (SX1262 maximum)
    pa_buf[2] = 0x00;  // DeviceSel: SX1262
    pa_buf[3] = 0x01;  // PALut (reserved, always 0x01)
    ...
    uint8_t tx_buf[2];
    tx_buf[0] = level;
    tx_buf[1] = 0x02;  // PA ramp: 40us (matches RNode CE firmware)
    executeOpcode(OP_TX_PARAMS_6X, tx_buf, 2);
}

int8_t SX1262::getTxPower() {
    return _txp;
}
...
// =============================================================================
// Internal Modulation & Packet Setup
// =============================================================================
...
void SX1262::onReceive(void(*callback)(int)) {
    ...
    if (callback) {
        ...
        uint8_t buf[8];
        buf[0] = 0xFF;  // Enable all IRQs
        buf[1] = 0xFF;
        buf[2] = 0x00;  // DIO0 mask: RX done
        buf[3] = IRQ_RX_DONE_MASK_6X;
        buf[4] = 0x00;  // DIO1: none
        buf[5] = 0x00;
        buf[6] = 0x00;  // DIO2: none
        buf[7] = 0x00;
